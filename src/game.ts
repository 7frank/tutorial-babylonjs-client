...
export default class Game {
  ...
  initNpcs(): void {
    this.room.state.npcs.onAdd((player, sessionId) => {
      const isCurrentPlayer = sessionId === this.room.sessionId;
      ...
      sphereMaterial.emissiveColor = BABYLON.Color3.FromHexString("#ff0000");
    ...
    this.room.state.npcs.onRemove((player, playerId) => {
      this.playerEntities[playerId].dispose();
      delete this.playerEntities[playerId];
      delete this.playerNextPosition[playerId];
    });
